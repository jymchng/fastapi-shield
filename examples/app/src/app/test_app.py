...
def test_login_valid_credentials():
    ...
    response = client.post(
        "/token", json={"username": "admin", "password": "admin_password"}
    )
    assert response.status_code == 200
    data = response.json()
...
def test_login_invalid_credentials():
    ...
    response = client.post(
        "/token", json={"username": "admin", "password": "wrong_password"}
    )
...
def test_get_current_user_no_token():
    ...
    assert response.status_code == 422, (
        ...
        response.json(),
    )  # Default error for shield failure
    assert response.json() == {
        "detail": [
            {
                "type": "missing",
                "loc": ["header", "x-api-token"],
                "msg": "Field required",
                "input": None,
            }
        ]
...
def test_get_current_user_with_token():
    ...
    response = client.get("/me", headers={"X-API-Token": "admin_token"})
    assert response.status_code == 200
    user = response.json()
...
def test_get_current_user_with_invalid_token():
    ...
    response = client.get("/me", headers={"X-API-Token": "invalid_token"})
...
def test_protected_products_admin_access():
    ...
    response = client.get("/protected/products", headers={"X-API-Token": "admin_token"})
    assert response.status_code == 200
    products = response.json()
    assert len(products) == len(PRODUCTS_DB)
...
def test_protected_products_user_access():
    ...
    response = client.get("/protected/products", headers={"X-API-Token": "user1_token"})
    assert response.status_code == 200
    products = response.json()
    assert len(products) == len(PRODUCTS_DB)
...
def test_protected_products_guest_access():
    ...
    response = client.get("/protected/products", headers={"X-API-Token": "guest_token"})
    assert response.status_code == 500  # Default error for shield failure
...
def test_create_product_admin():
    ...
    response = client.post(
        "/admin/products", json=new_product, headers={"X-API-Token": "admin_token"}
    )
...
def test_create_product_user():
    ...
    response = client.post(
        "/admin/products", json=new_product, headers={"X-API-Token": "user1_token"}
    )
    assert response.status_code == 500  # Default error for shield failure
...
def test_update_product_admin():
    ...
    response = client.put(
        "/admin/products/1", json=product_update, headers={"X-API-Token": "admin_token"}
    )
    assert response.status_code == 200
    product = response.json()
...
def test_delete_product_admin():
    ...
    response = client.delete(
        "/admin/products/1", headers={"X-API-Token": "admin_token"}
    )
...
def test_delete_product_user():
    ...
    response = client.delete(
        "/admin/products/2", headers={"X-API-Token": "user1_token"}
    )
    assert response.status_code == 500  # Default error for shield failure
