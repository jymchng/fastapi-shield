...
def test_get_current_user():
    ...
    assert response.json() == {'username': 'johndoe', 'email': 'johndoe@example.com', 'full_name': 'John Doe', 'scopes': ['read:profile']}
...
def test_get_items():
    ...
    access_token = token_response.json()["access_token"]
    token_type = token_response.json()["token_type"]
    response = client.get("/items", headers={"Authorization": f"{token_type} {access_token}"})
    assert response.status_code == 200
    assert response.json() == {"items": [{"item_id": 1, "name": "Foo"}, {"item_id": 2, "name": "Bar"}]}
...
def test_create_item():
    ...
    access_token = token_response.json()["access_token"]
    token_type = token_response.json()["token_type"]
    response = client.post("/items", headers={"Authorization": f"{token_type} {access_token}"}, json={"name": "Baz"})
    assert response.status_code == 200
...
def test_missing_scope():
    ...
    items_response = client.get("/items", headers={"Authorization": f"{token_type} {access_token}"})
    assert items_response.status_code == 403
    assert "Not enough permissions" in items_response.json()["detail"]
    
    profile_response = client.get("/users/me", headers={"Authorization": f"{token_type} {access_token}"})
    assert profile_response.status_code == 403
    assert "Not enough permissions" in profile_response.json()["detail"]
    
    create_response = client.post("/items", headers={"Authorization": f"{token_type} {access_token}"}, json={"name": "Baz"})
