...
async def validate_api_key(api_key: str) -> Optional[User]:
    ...
    if api_key == "valid-api-key":
        return User(
            username="api-key-user", email="apikey@example.com", auth_method="api_key"
        )
    return None
...
async def validate_session_cookie(session_id: str) -> Optional[User]:
    ...
    if session_id == "valid-session":
        return User(
            username="cookie-user", email="cookie@example.com", auth_method="cookie"
        )
    return None
...
def test_jwt_auth():
    ...
    assert response.json() == {
        "message": "Hello jwt-user! You are authenticated via jwt",
        "user": {
            "username": "jwt-user",
...
def test_api_key_auth():
    ...
    assert response.status_code == 200, response.json()
    assert response.json() == {
        "message": "Hello api-key-user! You are authenticated via api_key",
        "user": {
...
def test_session_cookie_auth():
    ...
    assert response.status_code == 200, response.json()
    assert response.json() == {
        "message": "Hello cookie-user! You are authenticated via cookie",
        "user": {
