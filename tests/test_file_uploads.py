    ...
    for file in files:
        try:
            ...
            try:
                ...
                content = Path(os.path.join(os.path.dirname(__file__), file.filename)).read_text()
...
def test_shielded_file_upload():
    ...
    with open(csv_file1_path, "rb") as csv_file1, open(csv_file2_path, "rb") as csv_file2:
        ...
        # Verify authentication failure
        assert response.status_code == 500, f"Expected 401, got {response.status_code}: {response.json()}"
        assert response.json() == {'detail': 'Shield with name `unknown` blocks the request'}, response.json()
