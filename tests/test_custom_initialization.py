...
class TestShieldFactoryPattern:
    ...
    def setup_method(self):
        ...
        def optional_admin_endpoint(user: Optional[dict] = ShieldedDepends(lambda x: x)):
    ...
    def test_admin_access(self):
        ...
        for endpoint in ["/admin", "/editor", "/viewer"]:
            response = self.client.get(endpoint, headers={"Authorization": "Bearer admin"})
            assert response.status_code == 200, f"Failed to access {endpoint} with admin: {response.text}"
            assert response.json()["message"] in ["Admin endpoint", "Editor endpoint", "Viewer endpoint"]
    ...
    def test_editor_access(self):
        ...
        for endpoint in ["/editor", "/viewer"]:
            response = self.client.get(endpoint, headers={"Authorization": "Bearer user2"})
            assert response.status_code == 200, f"Failed to access {endpoint} with editor: {response.text}"
    ...
    def test_viewer_access(self):
        ...
        for endpoint in ["/admin", "/editor"]:
            response = self.client.get(endpoint, headers={"Authorization": "Bearer user1"})
            assert response.status_code == 403
            assert "Access denied" in response.json()["detail"]
        
        response = self.client.get("/viewer", headers={"Authorization": "Bearer user1"})
        assert response.status_code == 200, f"Failed to access /viewer with viewer: {response.text}"
        assert response.json()["user"] == "user1"
    
    def test_invalid_token(self):
        """Test access with invalid token"""
        for endpoint in ["/admin", "/editor", "/viewer"]:
            response = self.client.get(endpoint, headers={"Authorization": "Bearer invalid-user"})
            assert response.status_code == 403
            assert "Access denied" in response.json()["detail"]
    ...
    def test_non_auto_error_shield(self):
        ...
        response = self.client.get("/optional-admin", headers={"Authorization": "Bearer admin"})
        ...
        response = self.client.get("/optional-admin", headers={"Authorization": "Bearer user1"})
...
class TestShieldClasses:
    ...
    def test_default_rate_limit(self):
        ...
        for i in range(5):
            ...
            assert response.json() == {"message": "Normal endpoint with standard rate limiting"}
    ...
    def test_strict_rate_limit(self):
        ...
        for i in range(2):
            ...
            assert response.status_code == 200
            assert response.json() == {"message": "Sensitive endpoint with strict rate limiting"}
    ...
    def test_rate_limit_reset(self):
        ...
        self.default_limiter.client_requests = {k: [] for k in self.default_limiter.client_requests}
...
class TestDependencyInjectionForConfig:
    ...
    def setup_method(self):
        ...
        def create_auth_shield():
            ...
            def auth_shield(api_token: str = Header(None, alias="x-api-token"), settings: SecuritySettings = Depends(get_security_settings)):
...
class TestDynamicShieldConfiguration:
    ...
    def test_allowed_token(self):
        ...
        response = self.client.get("/configurable", headers={"api-token": "admin_token"})
        assert response.status_code == 200
    ...
    def test_disallowed_token(self):
        ...
        response = self.client.get("/configurable", headers={"api-token": "invalid_token"})
