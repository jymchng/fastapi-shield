...
class TestLazyDependencyResolution:
    ...
    def setup_method(self):
        ...
        async def shield_endpoint(
            db=Depends(
                get_db_connection
            ),  # DB connection only created if shield passes
        ):
    ...
    def test_shield_valid_key(self):
        ...
        response = self.client.get(
            "/shield-protected", headers={"api-key": "valid_key"}
        )
    ...
    def test_shield_invalid_key(self):
        ...
        self.db_call_counter = 0
        response = self.client.get(
            "/shield-protected", headers={"api-key": "invalid_key"}
        )
...
class TestCleanEndpointSignatures:
    ...
    def setup_method(self):
        ...
        USERS = {
            "johndoe": User(
                username="johndoe", email="john@example.com", is_admin=False
            ),
            "adminuser": User(
                username="adminuser", email="admin@example.com", is_admin=True
            ),
        }
        ...
        async def shield_admin_endpoint(
            ...
            admin_user: User = ShieldedDepends(
                lambda user: user
            ),  # Optional access to authenticated user
        ):
    ...
    def test_traditional_endpoint_non_admin_access(self):
        ...
        response = self.client.get(
            "/traditional/admin", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_shield_endpoint_non_admin_access(self):
        ...
        token = self.create_token("johndoe")
        response = self.client.get(
            "/shield/admin", headers={"Authorization": f"Bearer {token}"}
        )
        assert response.status_code == 403
        assert response.json()["detail"] == "Admin access required"
...
class TestComposabilityAndReusability:
    ...
    def setup_method(self):
        ...
        def rate_limit_shield(
            max_requests: int, window_seconds: float, name: str = None
        ):
            ...
            def rate_limiter(request: Request):
                ...
                request_history[client_ip] = [
                    ts for ts in request_history[client_ip] if now - ts < window_seconds
                ]
...
class TestErrorHandling:
    ...
    def test_custom_response_shield(self):
        ...
        response = self.client.get(
            "/custom-response", headers={"api-key": "invalid_key"}
        )
        assert response.status_code == 403
