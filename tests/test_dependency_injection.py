...
from fastapi import FastAPI, Header, HTTPException, Depends, Body, Query, Request
...
class TestBasicShieldWithDependencies:
    ...
    def setup_method(self):
        ...
        @app.get("/profile")
        @auth_shield
        async def get_profile(user: dict = ShieldedDepends(get_user_data)):
    ...
    def test_valid_authentication(self):
        ...
        response = self.client.get(
            "/profile", headers={"Authorization": "Bearer valid_user_token"}
        )
    ...
    def test_admin_authentication(self):
        ...
        response = self.client.get(
            "/profile", headers={"Authorization": "Bearer valid_admin_token"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_malformed_header(self):
        ...
        response = self.client.get(
            "/profile", headers={"Authorization": "invalid_format"}
        )
        assert response.status_code == 500  # Shield blocks request
...
class TestShieldCompositionAndChaining:
    ...
    def setup_method(self):
        ...
        def require_role(required_role: str):
            ...
            def role_shield(
                payload: dict = ShieldedDepends(
                    lambda payload: payload
                ),  # Gets data from previous shield
            ) -> Optional[dict]:
    ...
    def test_admin_profile_success(self):
        ...
        response = self.client.get(
            "/admin-profile", headers={"Authorization": "Bearer valid_jwt_token"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_admin_profile_invalid_token(self):
        ...
        response = self.client.get(
            "/admin-profile", headers={"Authorization": "Bearer invalid_token"}
        )
...
class TestPydanticIntegration:
    ...
    def setup_method(self):
        ...
        @shield(
            name="User Validator",
            exception_to_raise_if_fail=HTTPException(
                status_code=400, detail="User validation failed"
            ),
        )
        def validate_user_shield(
        ...
        async def register_user(
            ...
            return {"message": "User registered successfully", "data": enriched_data}
    ...
    def test_minor_user_registration(self):
        ...
        user_data = {"username": "younguser", "email": "young@company.com", "age": 16}
        response = self.client.post("/register", json=user_data)
        assert response.status_code == 200
    ...
    def test_reserved_username_blocked(self):
        ...
        user_data = {"username": "admin", "email": "admin@company.com", "age": 25}
        response = self.client.post("/register", json=user_data)
    ...
    def test_invalid_email_domain(self):
        ...
        user_data = {"username": "testuser", "email": "test@gmail.com", "age": 25}
        response = self.client.post("/register", json=user_data)
        assert response.status_code == 400
        assert "User validation failed" in response.json()["detail"]
    ...
    def test_pydantic_validation_errors(self):
        ...
        user_data = {"username": "testuser", "email": "invalid-email", "age": 25}
        response = self.client.post("/register", json=user_data)
        assert response.status_code == 422

        # Age too young
        user_data = {"username": "testuser", "email": "test@company.com", "age": 10}
        response = self.client.post("/register", json=user_data)
        assert response.status_code == 422

        # Username too short
        user_data = {"username": "ab", "email": "test@company.com", "age": 25}
        response = self.client.post("/register", json=user_data)
        assert response.status_code == 422
...
class TestDatabaseIntegration:
    ...
    def test_admin_access_all_data(self):
        ...
        response = self.client.get("/user-data", headers={"api-key": "admin1_key"})
        assert response.status_code == 200
    ...
    def test_user_access_own_data(self):
        ...
        response = self.client.get("/user-data", headers={"api-key": "user1_key"})
        assert response.status_code == 200
    ...
    def test_inactive_user_blocked(self):
        ...
        response = self.client.get("/user-data", headers={"api-key": "inactive1_key"})
        assert response.status_code == 500  # Shield blocks request

    def test_invalid_api_key(self):
        """Test invalid API key is rejected"""
        response = self.client.get("/user-data", headers={"api-key": "invalid_key"})
        assert response.status_code == 500  # Shield blocks request
...
class TestAdvancedShieldPatterns:
    ...
    def setup_method(self):
        ...
        def feature_flag_shield(
            feature: str = Query(...), user_type: str = Header(default="regular")
        ) -> Optional[dict]:
        ...
        async def access_feature(
            feature_name: str, access_info: dict = ShieldedDepends(lambda info: info)
        ):
        ...
        def rate_limit_shield(x_client_id: str = Header()) -> Optional[dict]:
            ...
            return {"client_id": x_client_id, "rate_limit": rate_limits[x_client_id]}

        @app.get("/api/data")
        @rate_limit_shield
        async def get_api_data(client_info: dict = ShieldedDepends(lambda info: info)):
    ...
    def test_feature_access_regular_user(self):
        ...
        response = self.client.get(
            "/feature/new_ui?feature=new_ui", headers={"user-type": "regular"}
        )
        assert response.status_code == 200
    ...
    def test_rate_limit_valid_client(self):
        ...
        response = self.client.get("/api/data", headers={"x-client-id": "client1"})
        assert response.status_code == 200
    ...
    def test_rate_limit_invalid_client(self):
        ...
        response = self.client.get(
            "/api/data", headers={"x-client-id": "invalid_client"}
        )
...
class TestShieldFactoryPatterns:
    ...
    def setup_method(self):
        ...
        @auth_shield
        @admin_shield
        async def admin_only(user: dict = ShieldedDepends(lambda user: user)):
            return {"message": "Admin access granted", "user": user["username"]}

        @app.get("/editor-only")
        @auth_shield
        @editor_shield
        async def editor_only(user: dict = ShieldedDepends(lambda user: user)):
            return {"message": "Editor access granted", "user": user["username"]}

        @app.get("/user-data")
        @auth_shield
        async def user_data(user: dict = ShieldedDepends(lambda user: user)):
            return {
                "message": "User data",
                "user": user["username"],
                "roles": user["roles"],
    ...
    def test_admin_access_success(self):
        ...
        response = self.client.get("/admin-only", headers={"user-id": "admin1"})
        assert response.status_code == 200
    ...
    def test_admin_access_denied(self):
        ...
        response = self.client.get("/admin-only", headers={"user-id": "user1"})
        assert response.status_code == 403
    ...
    def test_editor_access_success(self):
        ...
        response = self.client.get("/editor-only", headers={"user-id": "editor1"})
        assert response.status_code == 200
    ...
    def test_editor_access_denied(self):
        ...
        response = self.client.get("/editor-only", headers={"user-id": "user1"})
        assert response.status_code == 403
    ...
    def test_user_data_access(self):
        ...
        response = self.client.get("/user-data", headers={"user-id": "user1"})
        assert response.status_code == 200
    ...
    def test_unauthenticated_access(self):
        ...
        response = self.client.get("/user-data", headers={"user-id": "nonexistent"})
...
class TestErrorHandlingPatterns:
    ...
    def setup_method(self):
        ...
        async def http_exception_handler(_: Request, exc: ValueError):
            return JSONResponse(
                status_code=500,
        ...
        @app.get("/test-errors")
        @custom_error_shield
        async def test_errors(data: dict = ShieldedDepends(lambda data: data)):
            return {"message": "Success", "data": data}

        self.app = app
        self.client = TestClient(app)

        @app.get("/normal-errors")
        async def test_normal_errors():
            raise TypeError("This is a normal TypeError.")
    ...
    def test_custom_error_response(self):
        ...
        response = self.client.get("/test-errors", headers={"test-mode": "fail"})
    ...
    def test_shield_internal_exception(self):
        ...
        response = self.client.get("/test-errors", headers={"test-mode": "exception"})
        assert response.status_code == 500
        assert "Internal shield error" in response.json()["detail"]

    def test_successful_shield_execution(self):
        """Test successful shield execution"""
        response = self.client.get("/test-errors", headers={"test-mode": "normal"})
        assert response.status_code == 200
